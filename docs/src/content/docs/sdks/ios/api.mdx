...
- Calls `ExperimentsAPI.fetchExperimentsAsync` which:
  ...
  - Performs network request and decodes `[Experiment]` into a map keyed by `experiment_key`
...
**Parameters:**
- `for experiment_key: String` - The experiment identifier (e.g., "/api/v1/home")
...
**Parameters:**
- `for experiment_key: String` - The experiment identifier
...
- **`ExperimentsCache.sessionVariables`** → Per-session variable cache, keyed by `experiment_key` and variable name
...
7. **Keep paths consistent**: Maintain `experiment_key` and variable names consistent with backend definitions
...
```swift
experiments.fetchExperiments(
    for: [experiment_key: ExperimentVariable],
