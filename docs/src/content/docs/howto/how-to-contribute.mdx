...
│   └── Ascend.podspec
│
├── ascend-react-native/              # React Native SDK
│   ├── src/
│   ├── android/
│   ├── ios/
│   ├── example/
...
```bash
git clone https://github.com/dream-horizon-org/ascend-android.git
cd ascend-android
```

#### 2. Open in Android Studio

- File → Open → Select `ascend-android` directory
...
```bash
git clone https://github.com/dream-horizon-org/ascend-react-native.git
cd ascend-react-native
```
...
**React Native:**
```bash
# Use local package in example app
cd example
yarn install
# The example app already references the local package
```
